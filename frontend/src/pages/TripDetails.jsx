...
function TripDetails() {
  ...
  // --- SAFE MATH FOR READ-ONLY RENDER ---
  ...
  // Bill specific logic (No holding charge shown in exact bill)
  const exactBillTotalFreight = freight + loadingCharge;
    ...
    <div className="p-4 md:p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="print:hidden flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-200">
         <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                    <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold border ${trip.actual_delivery_date ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                        {trip.actual_delivery_date ? 'Completed' : 'Active'}
                    </span>
                    <span className="px-2.5 py-0.5 rounded text-[10px] font-bold border bg-slate-50 text-slate-600 border-slate-200 uppercase tracking-wider">
                        Read-Only View
                    </span>
                </div>
             </div>
         ...
         <div className="flex items-center gap-2 w-full sm:w-auto bg-gray-100 p-1 rounded-xl">
             <button onClick={() => setViewType('receipt')} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2 rounded-lg font-bold transition cursor-pointer ${viewType === 'receipt' ? 'bg-white text-slate-900 shadow-sm border border-gray-200' : 'text-gray-500 hover:text-slate-700'}`}>
                 <Receipt className="h-4 w-4"/> Internal Receipt
             </button>
             <button onClick={() => setViewType('bill')} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2 rounded-lg font-bold transition cursor-pointer ${viewType === 'bill' ? 'bg-blue-600 text-white shadow-sm' : 'text-gray-500 hover:text-slate-700'}`}>
      ...
      <div className="bg-white shadow-lg border border-gray-200 rounded-2xl overflow-hidden print:shadow-none print:border-none print:rounded-none print:overflow-visible">
        <div ref={receiptRef} className="p-8 print:p-0 bg-white">
          
          {/* ========================================= */}
          {/*    INTERNAL READ-ONLY RECEIPT VIEW        */}
          {/* ========================================= */}
          {viewType === 'receipt' && (
            <div className="max-w-3xl mx-auto">
              ...
              <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex justify-between items-center border-b border-gray-100 pb-2 mb-2">
                        ...
                        <span className="font-bold text-slate-800">{freight}</span>
                    </div>
                    {loadingCharge > 0 && (
                        <div className="flex justify-between items-center border-b border-gray-100 pb-2 mb-2">
                            <span className="text-sm font-semibold text-gray-700">Loading/Unloading (₹)</span>
                            <span className="font-bold text-slate-800">{loadingCharge}</span>
                        </div>
                    )}
                    {holdingCharge > 0 && (
                        <div className="flex justify-between items-center border-b border-gray-100 pb-2 mb-2">
                            <span className="text-sm font-semibold text-gray-700">Holding Charge (₹)</span>
                            <span className="font-bold text-slate-800">{holdingCharge}</span>
                        </div>
                    )}
                    {gstAmount > 0 && (
                        <div className="flex justify-between items-center border-b border-gray-100 pb-2 mb-2">
                            <span className="text-sm font-bold text-gray-900">GST (18%) (₹)</span>
                            <span className="font-bold text-emerald-600">{gstAmount}</span>
                        </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {tds > 0 && (
                        <div className="flex justify-between items-center border-b border-gray-100 pb-2 mb-2">
                            ...
                            <span className="font-bold text-rose-600">{tds}</span>
                        </div>
                    )}
                    {extraDeduction > 0 && (
                        <div className="flex justify-between items-center border-b border-gray-100 pb-2">
                            <span className="text-sm font-semibold text-gray-700">Extra Deduction</span>
                            <span className="font-bold text-rose-600">{extraDeduction}</span>
                        </div>
                    )}
                  </div>
                  ...
                  <div className="col-span-2 mt-4 p-4 border-2 border-slate-900 rounded-lg flex justify-between items-center bg-emerald-50/30">
                      ...
                      <span className="font-extrabold text-2xl text-emerald-600">₹{balancePayment}</span>
                  </div>
              </div>
              ...
              {/* Read-Only Operational Tracking */}
                  ...
                  <div className="border border-black flex justify-between">
                      ...
                      <div className="w-[40%] text-[13px] font-bold">
                          <div className="border-b border-black p-2 flex justify-between">
                              ...
                              <span>{trip.bill_no || 'JTA/26-27/---'}</span>
                          </div>
