...
function Trips() {
  ...
  const fetchTrips = async () => {
    try { const res = await axios.get(`${API_BASE}/trips/active`); setActiveTrips(res.data); }
    catch (err) { console.error("Error fetching trips:", err); }
  };

  const fetchAvailableTrucks = async () => {
    try { const res = await axios.get(`${API_BASE}/assets`); setAvailableTrucks(res.data.filter(t => t.current_status !== 'In-Transit')); }
  ...
  const fetchParties = async () => {
    try { 
        ...
        setParties(res.data.filter(Boolean)); 
    } catch (err) { console.error("Error fetching parties:", err); }
  };

  const handleAddTrip = async () => {
    if(!tripData.vehicle_number || !tripData.party_name) { alert("Vehicle & Party Name required!"); return; }
    try {
      await axios.post(`${API_BASE}/trips`, tripData);
      alert("Trip Launched!");
      fetchTrips();
      fetchParties();
    } catch (err) { alert("Error launching trip."); }
  };
      ...
      <section className="bg-white p-6 rounded shadow border">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <select className="border p-2 rounded" onChange={e => {
            if(e.target.value === "Other") { setIsOther(true); setTripData({...tripData, party_name: ''}); }
          ...
          {isOther && <input className="border p-2 rounded col-span-2" placeholder="Enter New Party Name" onChange={e => setTripData({...tripData, party_name: e.target.value})} />}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <select className="border p-2" onChange={(e) => setPodUpdate({...podUpdate, pod_status: e.target.value})}>
              <option value="Pending">Pending</option><option value="Received">Received</option><option value="Forwarded">Forwarded</option>
            </select>
