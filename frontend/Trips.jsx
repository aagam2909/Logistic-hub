...
function Trips() {
  ...
  const [tripData, setTripData] = useState({
    ...
    eway_bill: '', eway_bill_expiry: '', trip_start_date: '', lw: ''
  });
  ...
  const handleAddTrip = async () => {
    // MANDATORY FIELDS CHECK: Only Truck Number, Source, and Destination are required
    if (!tripData.vehicle_number.trim() || !tripData.source_city.trim() || !tripData.destination_city.trim()) {
      alert("Please fill in the mandatory fields: Truck Number, Source, and Destination!");
      return;
    }
    ...
    try {
      ...
      alert(err.response?.data?.detail || "Error launching trip.");
    }
  };
  ...
  const handleCompleteTrip = async (trip_id) => {
    ...
    try {
        ...
        alert("POD Uploaded & Trip Completed! Truck is now available.");
        fetchTrips(); 
        fetchAvailableTrucks();
      ...
      <section className="bg-white p-6 rounded shadow border">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <input 
            ...
            onChange={e => setTripData({...tripData, source_city: e.target.value})} 
          />
          ...
          <input 
            ...
            onChange={e => setTripData({...tripData, destination_city: e.target.value})} 
          />
          ...
          <input className="border p-2 rounded" placeholder="GTA Name" value={tripData.gta_name} onChange={e => setTripData({...tripData, gta_name: e.target.value})} />
          <input className="border p-2 rounded" placeholder="LR No" value={tripData.lr_no} onChange={e => setTripData({...tripData, lr_no: e.target.value})} />
          
          <div>
            <label className="block text-xs text-gray-500 mb-1">E-way Bill Date</label>
            <input className="border p-2 rounded w-full" type="date" value={tripData.eway_bill} onChange={e => setTripData({...tripData, eway_bill: e.target.value})} />
          </div>

          <div>
            <label className="block text-xs text-gray-500 mb-1">Launch Date</label>
            <input className="border p-2 rounded w-full" type="date" value={tripData.trip_start_date} onChange={e => setTripData({...tripData, trip_start_date: e.target.value})} />
          </div>

          <input className="border p-2 rounded col-span-3" placeholder="L/W Details (Optional)" value={tripData.lw} onChange={e => setTripData({...tripData, lw: e.target.value})} />
          
          <button onClick={handleAddTrip} className="bg-green-600 text-white p-2 rounded col-span-3 font-bold hover:bg-green-700">Launch Route 🚀</button>
        </div>
      </section>
      ...
      <section className="bg-white p-6 rounded shadow border">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <input type="date" className="border p-2 rounded" onChange={(e) => setPodUpdate({...podUpdate, pod_arrived_office_date: e.target.value})} />
            <input type="date" className="border p-2 rounded" onChange={(e) => setPodUpdate({...podUpdate, pod_forwarded_client_date: e.target.value})} />
          </div>
          <button onClick={handleUpdatePOD} className="bg-blue-600 text-white px-4 py-2 rounded font-bold hover:bg-blue-700">Update POD 📄</button>
        </div>
        ...
        <div className="overflow-x-auto mt-6">
          <table className="w-full text-sm">
            <thead className="bg-gray-100">
              <tr>
                <th className="p-2 text-left">Trip ID / Tracking</th>
                <th className="p-2 text-left">Status</th>
                <th className="p-2 text-left">Office Arrival</th>
                <th className="p-2 text-left">Forwarded</th>
                <th className="p-2 text-left">Upload POD</th>
            ...
            <tbody>
              {activeTrips?.map?.(trip => (
                <tr key={trip.trip_id} className="border-t">
                  <td className="p-2">
                    <a href={`/trip-details/${trip.trip_id}`} className="text-blue-600 underline font-bold hover:text-blue-800">
                  ...
                  <td className="p-2"><button onClick={() => handleCompleteTrip(trip.trip_id)} className="bg-blue-600 text-white px-3 py-1 rounded hover:bg-blue-700">Complete</button></td>
                </tr>
              ))}
              {!activeTrips.length && (
                <tr><td className="p-4 text-center text-gray-500" colSpan="6">No active trips found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
